...
export const BoardProvider = ({ children }) => {
  ...
  // 4. posts 상태 변경 시 localStorage에 저장 (이전과 동일)
  useEffect(() => {
    localStorage.setItem("posts", JSON.stringify(posts));
  }, [posts]);

  // ⭐️ 5. allUserLikes가 변경될 때마다 localStorage에 저장하고, 현재 사용자의 likes 상태도 업데이트합니다.
  useEffect(() => {
  ...
  const togglePostLike = (postId) => {
    ...
    setAllUserLikes(prevAll => {
      ...
      const isCurrentlyLiked = !!currentUserLikes[postId];
  ...
  return (
    <BoardContext.Provider value={{ 
      ...
      updateBoard, 
      deleteBoard, 
      likes, // ⭐️ 현재 사용자의 좋아요 상태만 노출
      togglePostLike // 좋아요 토글 함수 제공
    }}>
      {children}
