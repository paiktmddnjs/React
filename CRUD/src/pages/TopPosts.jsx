...
function TopPosts() {
  ...
  const topPosts = useMemo(() => {
    // ⭐️ 안정성 보강: postLikeCounts가 null/undefined일 경우 빈 객체로 처리
        ...
        <List>
          {topPosts.length === 0 ? (
            ...
            topPosts.map((post, index) => (
              // ⭐️ PostCard를 styled(Link)로 수정했으므로, 
              // 상세 페이지 링크를 to prop으로 전달합니다.
              <PostCard 
                key={post.id} 
                to={`/board/${post.id}`} 
                $rank={index + 1}
              >
                ...
                {/* ⭐️ ViewDetailLink 대신 PostCard 전체가 링크 역할을 하므로 제거합니다.
                    필요하다면 아래 주석처럼 ViewDetailLink를 PostCard 내부에 사용하여 시각적인 버튼을 만들 수 있습니다. */}
                {/* <ViewDetailLink to={`/board/detail/${post.id}`}>
                  자세히 보기 &rarr;
                </ViewDetailLink> */}
              </PostCard>
            ))
